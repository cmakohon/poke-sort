 ...
 */
contextBridge.exposeInMainWorld("mault", {
  isDesktop: true,
  serial: {
    getPreferredPortId: (): Promise<string | null> =>
      ipcRenderer.invoke("mault:get-preferred-serial-port"),
    setPreferredPortId: (portId: string | null): Promise<string | null> =>
      ipcRenderer.invoke("mault:set-preferred-serial-port", portId),
    onPortsChanged: (listener: () => void) => {
      const handler = () => listener();
      ipcRenderer.on("mault:serial-ports-changed", handler);
      return () => ipcRenderer.removeListener("mault:serial-ports-changed", handler);
    },
  },
});
